    ...
    public void serialize(T dateTime, JsonGenerator jsonGenerator, SerializerProvider serializerProvider) throws IOException {
        if (dateTime != null) {
            if (dateTime instanceof ZonedDateTime date) {
                ...
                jsonGenerator.writeString(StringUtil.localDateTime(date.atOffset(ZoneOffset.UTC).toLocalDateTime()));
            } else if (dateTime instanceof Date date) {
                jsonGenerator.writeString(StringUtil.localDateTime(date.toInstant().atOffset(ZoneOffset.UTC).toLocalDateTime()));
            }
        }
    }
}